...
class CRUDApp(tb.Window):
    ...
    def create_widgets(self): # function to create the widgets for the entry frame and buttons for the button frame
        ...
        self.todo_status = tb.Radiobutton(self.entry_frame, textvariable=self.status_var, value="To Do")
        self.ongoing_status = tb.Radiobutton(self.entry_frame, textvariable=self.status_var, value="Ongoing")
        self.done_status = tb.Radiobutton(self.entry_frame, textvariable=self.status_var, value="Done")
    ...
    def create_task(self):
        ...
        # messagebox.showinfo('info', f"{type(due_date)}")
    ...
    def update_task(self):
        ...
        if id is None or id not in self.db.read_all():
            ...
            self.title_var.set(self.db.read_all()[id][1])
            self.description_var.set(self.db.read_all()[id][2])
            self.priority_var.set(self.db.read_all()[id][3])
            self.status_var.set(self.db.read_all()[id][4])
            self.due_date_var.set(self.db.read_all()[id][6])
            self.db.update(id, self.title_var.get(), self.description_var.get(), self.priority_var.get(), self.status_var.get(), self.due_date_var.get())
